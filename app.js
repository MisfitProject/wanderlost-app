...
function init() {
    ...
    btnScan = document.getElementById('nav-discover-btn');
    ...
    // Safe Event Binding Helper (Standard function for hoisting/scope stability)
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                    ...
                    await showAlert(`Unable to reach the Intelligence Rig.`, "Connection Error", "fa-tower-broadcast");
                    if (btnScan) {
                        if (discoverIcon) discoverIcon.className = 'fa-solid fa-satellite-dish';
                        if (statusText) statusText.textContent = 'Scanning for nearby secrets...';
                        btnScan.disabled = false;
            ...
            showAlert("Geolocation is not supported.", "Device Error", "fa-mobile-screen");
            if (btnScan) btnScan.disabled = false;
        }
    }
    window.startScan = startScan; // Export for global access if needed
