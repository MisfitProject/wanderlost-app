...
app.post('/api/discover', async (req, res) => {
    ...
    try {
        ...
        // 2. Pick a candidate and fetch reviews
        // CRITICAL FIX: The New Places API (v1) returns id as 'places/PLACE_ID'
        // The Legacy Place Details API needs ONLY the PLACE_ID string.
        ...
        try {
            const aiResponse = await axios.post(AI_RIG_URL, {
                ...
                prompt: `You are an expert cultural scout. Analyze these reviews for a place:
                
                Reviews:
                ${reviewText}
                
                Criteria:
                1. Is it a "local gem" (not a tourist trap)?
                2. Are at least 90% of reviews likely from locals/natives (Check language, depth of local knowledge, lack of generic tourist phrases)?
                
                Respond ONLY with JSON: {"isLocal": true/false, "reason": "1-sentence explanation of why it fits/doesn't fit"}.`,
                stream: false,
                format: 'json',
